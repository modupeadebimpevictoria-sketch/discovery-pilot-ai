...
export default function CareerPassport() {
  ...
  const handleCopyLink = () => {
    const summaryText = [
      `🎒 ${profile?.name || "Student"}'s Findr Career Passport`,
  ...
  const handleDownloadPDF = () => {
    ...
    lines.push(hr);
    lines.push(`Generated by Findr • ${new Date().toLocaleDateString()}`);
    ...
    a.href = url;
    a.download = `Findr_Passport_${profile?.name || "Student"}.txt`;
    a.click();
